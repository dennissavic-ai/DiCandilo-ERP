...
export const userRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/', { preHandler: [authenticate, requireAdmin] }, async (request, reply) => {
    try {
      ...
      const user = await prisma.user.create({
        data: { companyId, ...body, email: body.email.toLowerCase(), passwordHash, createdBy: sub, updatedBy: sub },
        select: { id: true, email: true, firstName: true, lastName: true, role: { select: { id: true, name: true } } },
      });
  ...
  fastify.get('/roles', { preHandler: [authenticate] }, async (_request, reply) => {
    try {
